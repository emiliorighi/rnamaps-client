...
<div class="flex-parent">
	<div class="input-flex-container">
		<div v-for="item in items" :key="item.time" :ref="item.time" :class="item.active? 'input active':'input'" style="text-align: initial"  @click="activateItem(item)">
			<span data-year="33"  :data-info="item.time"></span>
		</div>
	</div>
</div>
</template>
<script>
import {humanTimePoints} from '../static'
  export default {
    data: () => ({
      items: humanTimePoints
    }),
	methods: {
		activateItem(item){
			this.items.filter(it => it.time !== item.time && it.active).forEach(it => it.active = !it.active)
			item.active = !item.active
			// console.log(key)
			// console.log(this.$refs[key][0])
			// this.$refs[key][0].classList.value = this.$refs[key][0].classList.value + ' active'
		}
	}
  }
</script>
